...
bool DS3231_GetTime(_RTC *rtc)
{
  ...
  //if(HAL_I2C_Master_Transmit(i2c, DS3231_ADDR, 0x00, 1, HAL_MAX_DELAY) != HAL_OK) return false;
      //if(HAL_I2C_Master_Receive(i2c, DS3231_ADDR, buffer, 1, HAL_MAX_DELAY) != HAL_OK) return false;

            // --> read seconds only

  rtc->Sec = B2D(buffer[0] & 0x7F);  // masking to read only necessary information
